...
drone_position actual_position = {.x = 30, .y = 30};			// actual drone position
...
drone_position landed = {.x = MAX_X * 2, .y = MAX_Y * 2}; // position for 'landed' status (i.e. when the drone is in charge)
int steps = 0;
int dir_changer;
...
void recharging()
{
	CHECK(write(sockfd, &landed, sizeof(drone_position))); // communicates to the master that the drone is recharging ('landed' status)
	attron(A_BOLD);
	attron(COLOR_PAIR(3));
