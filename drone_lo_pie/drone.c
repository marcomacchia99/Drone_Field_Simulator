...
int main(int argc, char * argv[]){
    ...
    while(1){
        ...
        if(cicli == random){ // To change the exploration direction after a random number of loop beetween 0 and 20.
            direction = !direction;
            cicli = 0;
            random = rand() % 20;
        }
        ...
        if(instruction == 0){ // Check the instruction received from the master if it is 0 movement is not allowed.
            ...
            attron(COLOR_PAIR(2));
            mvaddstr(43, 0, "Movement not allowed.");
...
void recharge(int fd){
    ...
    char x = 219; // char for loading bar 

    for(int i=0; i< 50; i++){ // for loop for loading bar 
    attron(COLOR_PAIR(2));
    mvaddstr(41, 0, "                                      ");
    mvaddstr(42, 0, "Charging in progres...");
    mvaddch(43, i, x); // add char x at y = 43 and x = i to print the loading bar
    attroff(COLOR_PAIR(2));
    refresh();
    usleep(100000);
    }
